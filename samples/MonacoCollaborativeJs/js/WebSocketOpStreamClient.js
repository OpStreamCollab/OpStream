...
export class WebSocketOpStreamClient {
                ...
                switch (message.messageType) {
                    ...
                    case "ReceiveOpEvent":
                        if (this.onReceiveOp && message.receiveOpEvent) {
                            // The C# uses byte[], which System.Text.Json serializes as Base64 string.
                            // We return it as is (base64 string in JS).
    ...
    /**
     * Helper to wait for the WebSocket to connect
     */
    async _waitForConnection() {
        if (!this._webSocket) return;

        // Ready state constants: 0 = CONNECTING, 1 = OPEN, 2 = CLOSING, 3 = CLOSED
    ...
    async connectAndJoinAsync(documentId, documentType) {
        ...
        return {
            ...
            snapshot: jr.snapshot, // Base64 string
    ...
    async sendOpAsync(documentId, payload, baseRevision) {
        ...
        // System.Text.Json natively treats byte arrays as base64 encoded strings
    ...
    async sendAwarenessAsync(documentId, data) {
        ...
        // No correlation ID needed, one-way message
    ...
    async listOpenCommentsAsync(documentId) {
        const response = await this._sendCommentRequestAsync(
            this.MessageType.ListOpenComments,
            { documentId: documentId }
        );
    ...
    async createCommentAsync(documentId, cmd) {
        const response = await this._sendCommentRequestAsync(
            this.MessageType.CreateComment,
            {
                documentId: documentId,
                body: cmd.body || cmd.Body,
                parentCommentId: cmd.parentCommentId || cmd.ParentCommentId,
                anchor: cmd.anchor || cmd.Anchor || null
            }
        );
    ...
    async editCommentAsync(documentId, commentId, newBody) {
        const response = await this._sendCommentRequestAsync(
            this.MessageType.EditComment,
            { documentId: documentId, commentId: commentId, body: newBody }
        );
    ...
    async resolveCommentAsync(documentId, commentId) {
        const response = await this._sendCommentRequestAsync(
            this.MessageType.ResolveComment,
            { documentId: documentId, commentId: commentId }
        );
    ...
    async deleteCommentAsync(documentId, commentId) {
        await this._sendCommentRequestAsync(
            this.MessageType.DeleteComment,
            { documentId: documentId, commentId: commentId }
        );
    }

    async _sendCommentRequestAsync(messageType, cmd) {
        const correlationId = this._generateId();
        const request = {
            correlationId: correlationId,
        ...
        if (response.messageType === this.MessageType.ErrorResponse || response.messageType === "ErrorResponse") {
            throw new Error(response.errorMessage || "Comment operation failed.");
        }
